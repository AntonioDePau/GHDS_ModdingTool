...
namespace WAV
{
    ...
    class WAVFile
    {
        ...
        public String Open(String pFilename, WAVFileMode pMode)
        {
            ...
            byte[] bytes = File.ReadAllBytes(mFilename);
            return (Open(bytes, pMode));
        }
        
        public String OpenBytes(byte[] bytes, WAVFileMode pMode)
        {
            mFilename = "dummy";
            return (Open(bytes, pMode));
        }
        ...
        /// /// <param name="pMode">The file opening mode.  Only READ and READ_WRITE are valid.  If you want to write only, then use Create().</param>
        /// <returns>A blank string on success, or a message on failure.</returns>
        public String Open(byte[] bytes, WAVFileMode pMode)
        {
            ...
            try
            {
                mMemoryStream = new MemoryStream(bytes);
                mFileMode = pMode;

                // RIFF chunk (12 bytes total)
                // Read the header (first 4 bytes)
                byte[] buffer = new byte[4];
                mMemoryStream.Read(buffer, 0, 4);
                ...
                mEncodingType = BitConverter.ToInt16(buffer, 0);
                // # of channels (2 bytes)
                mMemoryStream.Read(buffer, 2, 2);
        ...
        {
            if(mMemoryStream != null && mBinaryWriter != null)
            {
                ...
                if ((mFileMode == WAVFileMode.WRITE) || (mFileMode == WAVFileMode.READ_WRITE))
                {
                    ...
                    if (!BitConverter.IsLittleEndian)
                        Array.Reverse(buffer);
                    mBinaryWriter.Write(buffer, 0, 4);
        ...
        public byte[] GetNextSample_ByteArray()
        {
            ...
            try
            {
                ...
                mMemoryStream.Read(audioSample, 0, numBytes);
        ...
        public void AddSample_ByteArray(byte[] pSample)
        {
            if (pSample != null)
            {
                ...
                try
                {
                    ...
                    mBinaryWriter.Write(pSample, 0, numBytes);
        ...
        public void WriteAllBytes(String pFilename, bool pOverwrite)
        {
            if(!pOverwrite && File.Exists(pFilename))
            {
                throw new Exception("File already exists!");
            }
            byte[] bytes = mMemoryStream.ToArray();
            File.WriteAllBytes(pFilename, bytes);
        }
        ...
        public void Create(bool pStereo, int pSampleRate, short pBitsPerSample, bool pOverwrite)
        {
            ...
            try
            {
                ...
                mBinaryWriter.Write(buffer, 0, 2);
                // # of channels (2 bytes)
                Array.Clear(buffer, 0, buffer.GetLength(0));
                ...
                mBinaryWriter.Write(buffer_2bytes, 0, 2);
                // Bits per sample (2 bytes)
        ...
        /// <param name="pSampleRate">The sample rate of the audio file (in Hz)</param>
        /// <param name="pBitsPerSample">The number of bits per sample (8 or 16)</param>
        public void Create(bool pStereo, int pSampleRate, short pBitsPerSample)
        {
            Create(pStereo, pSampleRate, pBitsPerSample, true);
        }
        ...
        public static void AdjustVolume_Copy(String pSrcFilename, String pDestFilename, double pMultiplier)
        {
            ...
            if (retval == "")
            {
                ...
                destFile.Create(srcFile.IsStereo, srcFile.SampleRateHz, srcFile.BitsPerSample);
        ...
        public static void AdjustVolume_Copy_8BitTo16Bit(String pSrcFilename, String pDestFilename, double pMultiplier)
        {
            ...
            if (retval == "")
            {
                ...
                WAVFile destFile = new WAVFile();
                destFile.Create(srcFile.IsStereo, srcFile.SampleRateHz, 16, true);
        ...
        {
            CopyAndConvert(pSrcFilename, pDestFilename, -1, pBitsPerSample, pStereo, 1.0);
        }
        ...
        public static void CopyAndConvert(String pSrcFilename, String pDestFilename, int pSampleRateHz, short pBitsPerSample, bool pStereo, double pVolumeMultiplier)
        {
            WAVFile srcFile = new WAVFile();
            String retval = srcFile.Open(pSrcFilename, WAVFileMode.READ);
            if (retval != "")
                throw new WAVFileException(retval, "WAVFile.Convert_Copy()");
            WAVFile destFile = Convert(srcFile, pSampleRateHz, pBitsPerSample, pStereo, pVolumeMultiplier);
        }
        
        public static WAVFile Convert(WAVFile srcFile, int pSampleRateHz, short pBitsPerSample, bool pStereo, double pVolumeMultiplier)
        {
            WAVFile destFile = new WAVFile();
            if(pSampleRateHz == -1) pSampleRateHz = srcFile.SampleRateHz;
            destFile.Create(pStereo, srcFile.SampleRateHz, pBitsPerSample);
        ...
        {
            return true;
            /*return ((pSampleRateHz == 8000) || (pSampleRateHz == 11025) || (pSampleRateHz == 12000) ||
                    (pSampleRateHz == 16000) || (pSampleRateHz == 18900) ||
                    (pSampleRateHz == 22050) || (pSampleRateHz == 24000) || (pSampleRateHz == 28000) ||
                    (pSampleRateHz == 32000) || (pSampleRateHz == 37800) || (pSampleRateHz == 44056) ||
        ...
        public static void MergeAudioFiles(String[] pFileList, String pOutputFilename, String pTempDir)
        {
            ...
            for (int i = 0; i < pFileList.GetLength(0); ++i)
            {
                ...
                CopyAndConvert(pFileList[i], filename, -1, highestBitsPerSample, outputStereo, multiplier);
            ...
            outputFile.Create(outputStereo, firstFileAudioFormat.SampleRateHz,
        ...
        public void SeekToAudioSample(long pSampleNum)
        {
            if (mMemoryStream != null)
            {
                ...
                try
                {
                    mMemoryStream.Seek(mDataStartPos + (bytesPerSample * pSampleNum), 0);
                }
        ...
        /// <summary>
        /// Gets the encoding type for the audio file.  This is read from a field
        /// in the WAV file header.
        /// </summary>
        ...
        {
            get { return (mMemoryStream != null ? mMemoryStream.Position : 0); }
        }
        ...
        private int mEncodingType; // Encoding Type
